function OutputArg = LeastSquareLeveling(Observations,FirstPointHeight)
    ...
    Heights = zeros(NumberOfPoints,1);
    ...
    % If Observations is Nx4, a weighted least squares calculation should
    % be done
    if size(Observations,2) == 4
        W = diag((max(Observations(:,4))./Observations(:,4)).^2);
        X = (transpose(A)*W*A)\(transpose(A)*W*L);
    else
        X = (transpose(A)*A)\(transpose(A)*L);
    end
    % OutputArg is a MX1 matrix in which M is the number of points
    Heights(1,1) = FirstPointHeight;
    Heights(2:end,1) = X;
    OutputArg = Heights;
end
